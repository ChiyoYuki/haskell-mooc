...
readUntil f = readUntil' f []
  where readUntil' f res = do word <- getLine
                              case f word of True -> return res
                                             _ -> do words <- readUntil' f (res ++ [word])
                                                     return words
