...
distinct = (\t -> length t == length (nub t))
...
rangeOf = \t -> (last . sort $ t) - (head . sort $ t)
...
longest :: (Ord a) => [[a]] -> [a]
longest = \t -> longest' t []
  where longest' xs res = case xs of
                          [] -> res
                          x : y -> if (length x > length res) || (length x == length res && head x < head res)
                                   then longest' y x
                                   else longest' y res
...
incrementKey _ [] = []
incrementKey k (kv : kvs) 
  | (k == fst kv) = (fst kv , 1 + snd kv) : incrementKey k kvs
  | otherwise = kv : incrementKey k kvs
...
freqs xs = foldr freqs' (Map.fromList []) xs
  where freqs' x mapx = Map.alter freqs'' x mapx
        freqs'' Nothing = Just 1
        freqs'' (Just n) = Just (n + 1)
...
transfer from to amount bank 
  ...
  | amount < 0 = bank
  | otherwise = Map.alter (\(Just y) -> Just (y + amount)) to (Map.alter (\(Just x) -> Just (x - amount)) from bank)
...
maxIndex xs = foldr maxIndex' (fst (head xs')) xs'
  where maxIndex' pr ma = if snd pr > (xs ! ma) then fst pr else ma
        xs' = Data.Array.assocs xs
