...
appendAll ioref strs = do origin <- readIORef ioref
                          writeIORef ioref (origin ++ (foldr (++) "" strs))
...
swapIORefs x y = do xv <- readIORef x
                    yv <- readIORef y
                    zv <- readIORef x
                    writeIORef x yv
                    writeIORef y zv
...
hFetchLines h = do eof <- hIsEOF h
                   if eof
                   then return []
                   else do thisLine <- hGetLine h
                           remainLine <- hFetchLines h
                           return (thisLine : remainLine)
...
hSelectLines h nums = do hfl <- hFetchLines h
                         return (exact nums hfl)
  where exact [] _ = []
        exact (n : ns) hfl = if n > length hfl
                             then exact ns hfl
                             else (hfl !! (n - 1)) : exact ns hfl
...
interact' f state = do input <- getLine
                       let (continue, output, newst) = f (input, state)
                       putStrLn output
                       if continue
