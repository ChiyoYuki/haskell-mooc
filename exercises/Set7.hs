...
member x (Set s) = foldr (func x) False s
  where func x y z = y == x || z

-- add a member to a set
add :: Ord a => a -> Set a -> Set a
add x (Set s) = Set (add' x s)
  where add' x [] = [x]
        add' x (y : ys) = case compare x y of
                            LT -> x : y : ys
                            GT -> y : add' x ys
                            EQ -> y : ys
...
step s e = case s of
  Start -> if e == AddEggs then Eggs else Error
  Eggs -> case e of
          AddFlour -> Flour
          AddSugar -> Sugar
          _        -> Error
  Flour -> if e == AddSugar then FlourSugar else Error
  Sugar -> if e == AddFlour then FlourSugar else Error
  FlourSugar -> if e == Mix then Mixed else Error
  Mixed -> if e == Bake then Finished else Error
  _ -> s
...
reverseNonEmpty (x :| xs) = (\t -> head t :| tail t) (reverse (x : xs))
...
instance Ord a => Semigroup (Set a) where
  (<>) (Set x) (Set y) = case x of
    [] -> Set y
    (x : xs) -> (<>) (Set xs) (add x (Set y))
...
passwordAllowed pwd req = case req of
  MinimumLength n -> n <= length pwd
  ContainsSome str -> case pwd of
                      [] -> False
                      (x : xs) -> elem x str || passwordAllowed xs req
  DoesNotContain str -> not (passwordAllowed pwd (ContainsSome str))
...
data Arithmetic = Number Integer | Add Arithmetic Arithmetic | Mul Arithmetic Arithmetic
  deriving Show
...
evaluate arith = case arith of
  Number x -> x
  Add x y -> evaluate x + evaluate y
  Mul x y -> evaluate x * evaluate y

render :: Arithmetic -> String
render arith = case arith of
  Number x -> show x
  Add x y -> "(" ++ render x ++ "+" ++ render y ++ ")"
  Mul x y -> "(" ++ render x ++ "*" ++ render y ++ ")"
