...
up (Position x y) = Position x (1 + y)
...
rgb col = case col of
  ...
  Green -> [0,1,0]
  Blue -> [0,0,1]
  Mix x y -> zipWith (\ z w -> (z + w) / 2) (rgb x) (rgb y)
...
fromList x = case x of
  [] -> Empty
  (y : ys) -> Pair (fst y) (snd y) (fromList ys)
...
fromBin :: Bin -> Int
fromBin x = fromBin' x 0 1
  where fromBin' y sum pow = case y of
                             End -> sum
                             I z -> fromBin' z (sum + pow) (pow * 2)
                             O z -> fromBin' z sum (pow * 2)

toBin :: Int -> Bin
toBin x = if x == 0 then (O End) else toBin' x End
  where toBin' x y = case x of
                     0 -> y
                     x -> if even x then O $ toBin' (div x 2) y else I $ toBin' (div x 2) y
