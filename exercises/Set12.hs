...
fmap2 fun = (\xs -> fmap (fmap fun) xs)

fmap3 :: (Functor f, Functor g, Functor h) => (a -> b) -> f (g (h a)) -> f (g (h b))
fmap3 fun = (\xs -> fmap (fmap (fmap fun)) xs)
...
instance Functor TwoList where
  ...
  fmap f (TwoNode x y (twoList)) = TwoNode (f x) (f y) (fmap f twoList)
...
count eq xs = foldr (\x -> if x == eq then (+1) else id) 0 xs
...
inBoth xs ys = foldr (\x -> if elem x ys then (x:) else id) [] xs
...
instance Foldable List where
  foldr _ emp Empty = emp
  foldr f emp (LNode x xs) = f x (foldr f emp xs)
...
instance Foldable TwoList where
  foldr _ emp TwoEmpty = emp
  foldr f emp (TwoNode x y twoList) = f x (f y (foldr f emp twoList))
...
sumTree :: Monoid m => Tree m -> m
sumTree Leaf = mempty
sumTree (Node x lt rt) = sumTree lt <> x <> sumTree rt
