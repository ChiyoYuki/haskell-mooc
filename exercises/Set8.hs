...
blendColor x y = Color (div (getRed x + getRed y) 2)
                       (div (getGreen x + getGreen y) 2)
                       (div (getBlue x + getBlue y) 2)

combine :: (Color -> Color -> Color) -> Picture -> Picture -> Picture
combine fun (Picture f1) (Picture f2) = Picture f
  where f (Coord x y) = fun (f1 (Coord x y)) (f2 (Coord x y))
...
rectangle x0 y0 w h = Shape (\ (Coord x y) -> x0 <= x && 
                                              x < x0 + w && 
                                              y0 <= y && 
                                              y < y0 + h)
...
paintSolid color shape@(Shape fs) base@(Picture fb) = Picture f
  where f pos = if fs pos then color else fb pos
...
paint pat@(Picture fp) shape@(Shape fs) base@(Picture fb) = Picture (\x -> if fs x then fp x else fb x)
...
instance Transform Fill where
  apply (Fill col) = (\x -> Picture (\y -> col))

data Zoom = Zoom Int
  deriving Show

instance Transform Zoom where
  apply (Zoom int) = (\pic -> zoom int pic)

data Flip = FlipX | FlipY | FlipXY
  deriving Show

instance Transform Flip where
  apply FlipXY f = flipXY f
  apply FlipX (Picture fun) = Picture (\(Coord x y) -> fun (Coord (-x) y))
  apply FlipY (Picture fun) = Picture (\(Coord x y) -> fun (Coord x (-y)))
...
instance (Transform a, Transform b) => Transform (Chain a b) where
  apply (Chain x y) = (apply x) . (apply y)
...
instance Transform Blur where
  apply _ (Picture fun) = Picture (\(Coord x y) -> averColor [fun (Coord x y),
                                                              fun (Coord (x + 1) y),
                                                              fun (Coord (x - 1) y),
                                                              fun (Coord x (y + 1)),
                                                              fun (Coord x (y - 1))])
    where averColor cols = Color (averRGB getRed cols) (averRGB getGreen cols) (averRGB getBlue cols)
          averRGB :: (Color -> Int) -> [Color] -> Int
          averRGB getRGB cols = div (sum (map getRGB cols)) (length cols)
...
instance Transform BlurMany where
  ...
  apply (BlurMany num) = (apply (BlurMany (num - 1))) . (apply Blur)
------------------------------------------------------------------------------
