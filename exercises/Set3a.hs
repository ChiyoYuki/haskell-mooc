...
capitalize = unwords . (map (\(x : y) -> toUpper x : y)) . words
...
joinToLength n strs = filter (\x -> length x == n)
                             [str1 ++ str2 | str1 <- strs, str2 <- strs]
...
sumRights x = sum (map (either (\x -> 0) id) x)
...
multiApp f gs x = f (multiApp' gs x)
  where multiApp' gs x = case gs of
                         [] -> []
                         (g : gs') -> ((g x) : (multiApp' gs' x))
...
interpreter commands = interpreter' 0 0 commands
  where interpreter' x y commands = case commands of
                                   [] -> []
                                   ("up" : cs) -> interpreter' x (y + 1) cs
                                   ("down" : cs) -> interpreter' x (y - 1) cs
                                   ("left" : cs) -> interpreter' (x - 1) y cs
                                   ("right" : cs) -> interpreter' (x + 1) y cs
                                   ("printX" : cs) -> show x : interpreter' x y cs
                                   ("printY" : cs) -> show y : interpreter' x y cs
