...
treeMax t = case t of
  Empty -> 0
  Node n left right -> max n $ max (treeMax left) (treeMax right)
...
allValues condition tree = case tree of
  Empty -> True
  Node n left right -> condition n && allValues condition left && allValues condition right
...
mapTree f t = case t of
  Empty -> Empty
  Node n left right -> Node (f n) (mapTree f left) (mapTree f right)
...
cull val tree = case tree of
  Empty -> Empty
  Node n left right -> if n == val then Empty else Node n (cull val left) (cull val right)
...
isOrdered t = case t of
  Empty -> True
  Node n left right -> (valAtRoot left == Nothing || valAtRoot left < Just n) && 
                       (valAtRoot right == Nothing || Just n < valAtRoot right) && 
                       isOrdered left && 
                       isOrdered right
...
walk ss (Node n left right) = case ss of
  [] -> Just n
  (x : xs) -> if x == StepL then walk xs left else walk xs right
...
set path val tree = case tree of
  Empty -> Empty
  Node x left right -> case path of
    [] -> Node val left right
    (p : ps) -> if p == StepL 
                then Node x (set ps val left) right 
                else Node x left (set ps val right)
...
search val t = search' val t []
  where search' val t path = case t of
                             Empty -> Nothing
                             Node x left right -> if x == val
                                                  then Just path
                                                  else case search' val left (path ++ [StepL]) of
